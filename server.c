...
int main (void)
{
  ...
  if (host_fd < 0){fprintf(2, "XN: Fatal Error: Listening Socket\r\n"); return -1;}
  ...
  while (n_fds = epoll_wait(ep_fd, &ev, 33333, -1))
  {
    ...
    for(n; n<n_fds ; n++)
    {
      ...
      {
        remote_fd = accept4(host_fd, (struct sockaddr*)&remote_addr, &(socklen_t){sizeof(remote_addr)}, SOCK_NONBLOCK);
        ...
        if (ev[n].events & EPOLLIN)
	{
	  ...
	  if (ev[n].events & EPOLLRDHUP){ remove(ev[n].data.fd, ep_fd);}
	}
	else
	{
